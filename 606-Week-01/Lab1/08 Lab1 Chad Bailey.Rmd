---
title: "Data606 Lab1 - Introduction to R and RStudio"
author: "Chad Bailey"
output:
  html_document:
    css: ./lab.css
...
```{r DATA606 Specific Addon, eval=FALSE}
##STUDENT-ADDED CODE, needed for course DATA606
  library(DATA606)
  startLab('Lab1')
##end STUDENT-ADDED CODE
```
...
```{r student response 2, eval=FALSE}
## Student Response for Exercise 2
    ## Yes, there is a trend in the number of girls baptized over the years.
    ## Overall, the trend is an increase over time. However, there is a 
    ## sharp decrease from 1640 to 1650 with a return to an upward trend 
    ## beginning around 1660.
...
```{r student response exercise 3 - plot, eval=TRUE}
## Student Response for Exercise 3, plot
  proportionBoys <- arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls)
  years <- arbuthnot$year
  plot(years, proportionBoys, type = "l")
```


```{r student response exercise 3 - description, eval=TRUE}
## Student Response for Exercise 3, description
    ## The plot shows the proportion of boys to be relatively consistent 
    ## across years with values ranging between roughly 0.50 and 53.6
```
...
## Student Responses to "On Your Own"
#### Initial Exploration of the Dataset
```{r explore present dataset, eval=TRUE}
  ## load dataset to a variable
  present <- source("more/present.R")
  
  ## get summary of new variable
  summary(present)
  
  ## load just the data to new variable; dropping metadata
  presentOnlyData <- present$value
  
  ## get summary of present only data
  summary(presentOnlyData)
  
  ## get count of rows
  nrows <- nrow(presentOnlyData)
  
  ## get count of columns
  ncols <- ncol(presentOnlyData)
  
  ## get min year
  minYear <- min(presentOnlyData$year)
  ## get max year
  maxYear <- max(presentOnlyData$year)
  
  
```

#### Student Responses to Questions 1 & 2
  1.1 The "present" dataframe has `r nrows` records and `r ncols` variables or dimensions [`r nrows`, `r ncols`].  
  1.2 The variable names are `r names(presentOnlyData)`.  
  1.3 The years included are `r minYear` to `r maxYear`.  
  
  2.1 The counts in the "present" dataset are much larger than the "Arbuthnot" dataset.  
  2.2 The datasets are not on the same scale. "Arbuthnot" is measuring counts within a large city while "present" is measuring a the thrid most populous country.

#### Student Responses to Question 3
  3.1 Plot boy-to-girl ratio using the "present" dataset.
```{r plot present boy-to-girl raito, eval=TRUE}
  year <- presentOnlyData$year
  boys <- presentOnlyData$boys
  girls <- presentOnlyData$girls

  plot(year, boys / girls, type = 'l')
  
```

  3.2 The plot of boy-to-girl ratio for the U.S. shows that Arbuthnot's observation about boys being born in greater proportion than girls does hold up in the U.S. However, the ratio in the "present" dataset is much closer to 1 than the "Arbuthnot" dataset and the "present" dataset has general downward trend.  

#### Student Responses to Question 4      
```{r}
  ## add computed column totaling births for boys and girls
  presentOnlyData$Total <- presentOnlyData$boys + presentOnlyData$girls
  
  ## find the max birth count
  maxBirthCount <- max(presentOnlyData$Total)
  
  ## subset present to the record having total birth count equal to max birth count
  yearMaxBirthCount <- presentOnlyData[which(presentOnlyData$Total == maxBirthCount), c("year")]
  

```
  4.1  The year with the most total number of births in the U.S. within the "present" dataset was `r yearMaxBirthCount`.
