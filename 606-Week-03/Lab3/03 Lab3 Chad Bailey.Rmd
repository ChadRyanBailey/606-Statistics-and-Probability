...
##  
##  a. A streak of length 1 means the player made one shot and then missed one shot.
##  b. A streak of length 0 means the player made consecutive misses
...
##  
##  a. The distribution of shots is right skewed 
##  b. The typical streak length is 0, meaning a miss after a previous miss.
##  c. The longest streas in this dataset was 4 shots
...
##
## The simulation of flipping an unfair coin 100 times was 20 times heads and 80 times tails.
...
##  4.a To make the 'sample' function reflect a shooting percentage of 45%, the
##      'prob' argument has to be added with values of c(0.45, 0.55)


##  4.b sim_unfair_coin <- sample(outcomes, size = 133, replace = TRUE, prob = c(0.2, 0.8))

##  4.c run adjusted sample for 133 shots
outcomes <- c("H", "M")
sim_basket <- sample(outcomes, size = 100, replace = TRUE, prob = c(0.2, 0.8))
```
...
## 1.a  The distribution of the simulated independent shooter with a shooting 
##      averge of 45% is right skewed 
## 1.b  The typical streak was length 0 
## 1.c  The longest streak of baskets was 3.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    ```{r on_your_own2}
##  2.  If the simulation was run again, it would most likely produce another right
##      skewed distribution. However, it could also produce a distribution that is 
##      exactly the same, similar, or entirely different. Similar is the most likely
##      because with a shooting percentage of 45%, near 50%, the shot is likely to 
##      be a miss roughly half the time and result in an end to the streak. Thus 
##      streaks end often.
...
##  3.a The distribution of Kobe's streak lengths is similar in shape to the 
##      simulated independent shooter though Kobe's is some what flatter.
##  3.b Comparing these two limited samples I would not feel there sufficient
##      evidence to support the "hot handle" claim. However, the somewhat
##      flatter and longer tail of for Kobe's distribution are interesting.
##      It would be fun to see multiple distributions for Kobe across his career
##      and multiple similuted distributions for a more informed comparison.
```
